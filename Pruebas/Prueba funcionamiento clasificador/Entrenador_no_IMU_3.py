...
data_file = os.path.join(path, 'Datos_50000_8_gestos_SIN_TRES.xlsx')
...
# Guardar el scaler
scaler_path = os.path.join(path_out, 'Escaladores\\scaler_50000_no_IMU_8_SIN_TRES.pkl')
joblib.dump(scaler, scaler_path)
print(f"Scaler guardado como '{scaler_path}'")

# Crear clasificador SVM con núcleo polinomial
model = SVC(C=0.1, coef0=1.0, degree=3, gamma='scale', kernel='poly')
...
model_path = os.path.join(path_out, 'Clasificadores\\clasificador_50000_no_IMU_8_SIN_TRES.pkl')
...
report = classification_report(y_test, y_pred, target_names=['BAJAR_DIAL', 'CRUZAR_DEDOS', 'FIST', 'GIRO_OUT', 'REST', 'UNO', 'WAVE_IN', 'WAVE_OUT'])
print("\nInforme de clasificación:")
print(report)

# Dibujar la matriz de confusión
ConfusionMatrixDisplay.from_estimator(model, X_test_scaled, y_test, display_labels= ['BAJAR_DIAL', 'CRUZAR_DEDOS', 'FIST', 'GIRO_OUT', 'REST', 'UNO', 'WAVE_IN', 'WAVE_OUT'])
