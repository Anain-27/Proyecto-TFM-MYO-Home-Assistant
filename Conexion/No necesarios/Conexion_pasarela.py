...
def limpieza(datos_emg):
    print("Limpiando datos EMG...")
    # Rectificación (valor absoluto) para los datos EMG
    emg_rectificado = np.abs(datos_emg)
    datos_emg = emg_rectificado

    # Escalado de los datos EMG
    datos_emg = datos_emg.reshape(1, -1)  # Reformar para que tenga la forma correcta
    datos_escalados = escalador.transform(datos_emg)
    print("Datos EMG limpiados y escalados.")
...
def trabajador_datos(modo, t_captura_total, t_espera, t_captura_cruzar_dedos):
    ...
    while recolectar:
        ...
        # Solo clasificar si los datos EMG están presentes
        if emg_data is not None:
            print("Datos EMG disponibles para clasificar.")
